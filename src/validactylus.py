...
spec = referencing.jsonschema.DRAFT202012 ## specification of JSON schema used


## currently (Apr. 2024) available topic schemas:
## ['data_mapping', 'data_observation', 'event', 'license', 'mapping', 'method',
## 'reference', 'sample', 'station']
...
parser.add_argument("url_instance", type = str,
                    ...
                    to be validated")
parser.add_argument("name_topic",
                    type = str, help = "name of a topic-specific schema")
...
assert (r_topic.status_code == 200,
        f"Couldn't retrieve schema {args['name_topic']}
        from {schema_base_url}{args['name_topic']}.json")

assert (r_shared.status_code == 200,
        f"Couldn't retrieve schema {args['name_shared']}
        from {schema_base_url}{args['name_shared']}.json")
